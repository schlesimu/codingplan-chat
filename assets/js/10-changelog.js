...
const CHANGELOG_DATA = [
  { v: 'v0.9.10.6', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '去掉输入框 .input-area 底部那团黑色发散阴影 — 三主题各有一处，dark/light/liquid 全清干净',
      'dark：删 0 8px 32px / 0 2px 8px 两条黑投影；保留顶部 1px 白细高光',
      'light：删 0 4px 20px / 0 1px 4px 两条黑投影；保留顶 1px 高光 + 彩虹边缘微光',
      'liquid：删 0 -4px 24px 紫光晕 + 0 -2px 8px 黑投影；保留全套 inset（液态玻璃核心）',
      '极简档（fx-minimal）一直没这阴影，所以泠说"除了极简都有"— 现在三档都干净了',
    ]},

  { v: 'v0.9.10.5', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '撤回 v0.9.10.4 的误删 — 那条 liquid 主题彩蛋诗的米色 text-shadow 不是泠想去掉的阴影，恢复它',
      '泠真正想去掉的阴影还在等定位，找准了再动',
    ]},

  { v: 'v0.9.10.4', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '去掉 liquid 主题彩蛋诗的米色 text-shadow — 那条提白边在液态玻璃背景上反而难看，干净的纯色字更耐看',
    ]},

  { v: 'v0.9.10.3', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '彩蛋诗句改为「拖拽跟随」交互：手指按住屏幕往上拽多少，那行诗就跟着手指走多少',
      '不再是阈值式一弹一弹，而是逐帧贴手 — 拽到一半停手就停在一半，松手按拉拽量自动留住或弹回',
      '拽过 40% 距离 → 完整露出停留 1.8s；拽不到 → 弹簧回弹（弹簧感）',
      '桌面端 wheel 行为保留：已到底继续向下滚仍然完整露出',
    ]},

  { v: 'v0.9.10.2', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '修复：底栏提示文「回车发送 · Shift+回车换行 · 粘贴图片 · 长按输入框说话」会被长按选中/复制 — 现已设为装饰性文字，pointer-events:none 穿透',
      '手指碰到那行提示时直接落到下方按钮上，不再误触系统选择菜单',
    ]},

  { v: 'v0.9.10.1', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '修复：长按输入框录音时，部分手机会同时弹出系统的「选择文本/复制」菜单 — 现已抑制',
      '抑制策略只在按下/录音期间生效，松手立即恢复，不影响平时复制粘贴 textarea 内容',
    ]},

  { v: 'v0.9.10.0', date: '2026-06-15',
    quote: '让说话和写字，回到同一只手里。',
    items: [
      '底栏改造：移除独立的麦克风按钮，把「按住说话」并入输入框本体',
      '输入框现在多面手：单击键盘打字，长按 0.4 秒进入语音录音转文字',
      '占位文从「输入你的问题…」改为「写下来 · 或按住说」',
      '录音态全底栏视觉：浅红覆盖 + 6 根实时波形 + 计时；上滑 80px 取消（底色加深 + 「松开取消」红色加粗）',
      '输入框宽度在小屏上多出约 30%，长 prompt 不再被挤到换行',
      '长按触发录音时会有 50ms 轻震动反馈（OPPO/Android 原生支持）',
      '通话页入口已迁至加号菜单，独立麦克风按钮的功能完全不丢失',
      '修复：顶栏的「来自小纸船」长条信封整体可点开明信片（之前只有 × 按钮响应）',
      '彩蛋诗句改为下拉到底再上拉显露：藏在屏幕外，聊天滑到底继续上拉 60px 才升起（顺手丢掉那道丑黑阴影）',
    ]},

  { v: 'v0.9.9.4', date: '2026-06-13',
    quote: '把多余的折回去，留下一艘船。',
    items: [
      '启动页大返工：从「拆礼物式三屏滑动」回到一页隐私同意，跟随当前主题，文案精简成正式条款风',
      '副标题改回一句话：一艘装着这个时代 AI 的船。',
      '欢迎气泡末句改成「想聊点什么？」',
      '新增信封寄语：顶栏正下方一条克制的纸感小条，只写一句寄语 — v0.9.9.4 是「把多余的折回去，留下一艘船」',
      '信封不强迫不打扰：当前版本一次「不再提示」永久关闭，下个大版本来时再唤醒一次',
      '关于页修无限下滑 bug，更新日志页设计语言对齐关于页',
      '视觉效果切换从「更多工具」里的全宽按钮升格为侧栏底部三段胶囊，紧贴主题切换器下方',
      '字体合规与分层：引入思源宋体（SIL OFL）做正文衬线兜底，UI 操作位（顶栏 / 侧栏 / 输入 / 按钮）保持系统无衬线，全设备视觉对齐',
      '修复一个老 bug：刷新页面不会再误闯启动页',
    ]},

  { v: 'v0.9.9.3', date: '2026-06-12',
    quote: '键也敲一下，鼠也点一下，都顺手。',
    items: [
      '桌面端键盘快捷键：⌘/Ctrl + K 新建对话、⌘/Ctrl + B 开关侧边栏、⌘/Ctrl + , 设置、⌘/Ctrl + / 显示快捷键面板',
      'Esc 智能关弹窗：栈式优先级，先关快捷键面板 → 再关高 z-index overlay → 最后关侧边栏',
      '↑ 键编辑上一条用户消息（仅在输入框为空时触发，否则放行光标移动）',
      '顶栏右上角新增 ⌨️ 快捷键面板入口（移动端不显示）',
      '面板自动检测系统：macOS 显 ⌘ 符号，Windows / Linux 显 Ctrl 符号',
      '面板里附输入框内置键 Enter / Shift+Enter 提示，方便新用户找到全部',
      '跨平台用 e.metaKey || e.ctrlKey 自动识别，不用判系统再写两套',
    ]},

  { v: 'v0.9.9.2', date: '2026-06-12',
    quote: '把这艘船的来路一笔一笔写清楚，再交到你手里。',
    items: [
      '收尾整理版：把启动页 / 反馈通道 / 史册 / 界面细节一次梳理干净',
      '启动页 3 屏从「黑紫夜空」改为「暖米白纸黄」基调，文字深棕、按钮暖橙',
      '版本号短按 → 寄语 toast，长按 → 反馈日志窗，含 UA / 视口 / 最近一条用户消息 / console + onerror 最多 500 行，一键复制或下载 .txt',
      '新增 00-console-capture.js，首发加载即拦截控制台输出，确保最早期报错能抓到',
      '更新日志补全早期 11 条：从 v0.3.2 一直回溯到 v0.1.0，史册扩到 73 条',
      '首尾以寄语包夹：v0.3.1「这艘船的最后一天还没有名字」/ v0.1.0「故事从这里开始」',
      '「关于小纸船」前 🚢 删除（保持文案克制）',
      '「语音 API 设置 🎙️」侧边栏按钮删除，改在 API Key 弹窗 TTS 区下方挂「高级语音 API 设置 →」按钮，保留全部功能',
      '输入框上方 5 个快捷气泡线性 SVG 图标全删，只保留 emoji + 文字（与表情一致性原则统一）',
    ]},

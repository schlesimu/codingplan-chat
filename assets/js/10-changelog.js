...
function showChangelog() {
  const changelog = [
    ...
    { v: 'v0.9.8.4', date: '2026-06-12', items: [
      ...
      '',
      '【✨ 输入栏改造】',
      '· 图片按钮 → 加号 ➕ 菜单（图片 / 文件 / 通话三选项）',
      '· 提示词模板按钮位置 → 新增 🎙️ 语音输入按钮',
      '· 提示词模板暂禁用（代码保留，待恢复）',
      '',
      '【🎙️ 语音输入（按住录音）】',
      '· 按住 🎙️ 按钮录音（上限 60 秒），松开自动转写',
      '· 走 doubao 多模态 ASR（默认 doubao-seed-1.6-flash）',
      '· 转写文字自动回填到输入框，可继续编辑后发送',
      '',
      '【📞 通话页（按住对讲版）】',
      '· 加号菜单 → 通话 → 进入全屏沉浸式通话页',
      '· 小纸船头像呼吸动画 + 渐变深空背景',
      '· 按住中间大圆按钮说话，松开自动转写 → 调 LLM → TTS 朗读回复',
      '· 实时显示对话转写（用户气泡 + 小纸船气泡）',
      '· 浏览器原生 TTS（默认中文音色）',
      '',
      '【⚙️ 更多工具 → 语音 API 设置】',
      '· ASR 服务商：豆包多模态（推荐）/ 火山豆包 ASR / 火山 RTC（v0.9.9）',
      '· ASR 模型可选：doubao-seed-1.6-flash / 1.6 / ark-code-latest / 自定义',
      '· API Key 留空 = 自动沿用主 LLM provider 的密钥，无需重复填',
      '· TTS 服务商：浏览器原生 / 火山 TTS（v0.9.9）/ 不朗读',
      '· TTS 语速可调（0.5–2.0）',
      '· 配置存 localStorage `codingplan-voice-config`',
      '',
      '【🌊 阶段 2 预告（v0.9.9）】',
      '· 火山 RTC 真·全双工实时对话（<1s 延迟 · 可打断 · AI 降噪）',
      '· 需在火山控制台开通 RTC + ASR + TTS 三件套',
      '· 接入官方开源 demo：volcengine/rtc-aigc-demo',
    ]},
    { v: 'v0.9.8.3', date: '2026-06-12', items: [
      '🚢 小纸船人格首发 + Coding 能力首发 —— 围绕「小纸船」语言体系做了文案重塑，输入栏新增提示词模板，代码块支持实时预览',
      '',
      '【🌊 D2-a — 加载文案换「小纸船划水」系列（8 处）】',
      '· typing-indicator 三点旁加文字「小纸船划水中…」（有呼吸渐变动效）',
      '· 联网搜索：「正在搜索：xxx」→「🌊 小纸船在打捞「xxx」…」',
      '· 思维链：「正在思考...」→「🧠 小纸船在打草稿…」',
      '· 分享上传：「上传中…」→「🚢 小纸船在折信封…」',
      '· 分享查看加载：「加载中…」→「🌊 小纸船归港中…」',
      '· 文件解析：「解析中...」→「小纸船拆包中…」',
      '· API Key 校验：「正在校验...」→「🌫️ 小纸船在测水深…」',
      '· 余额查询：「正在查询余额...」→「⛽ 小纸船在数灯油…」',
      '· 扫码解密：「正在解密配置...」→「✨ 小纸船在拆密信…」',
      '',
      '【🌫️ D2-b — 错误提示纸船化（6 场景）】',
      '· 网络出错：「网络出错：xxx」→「🌫️ 小纸船迷航了：xxx，请重试」',
      '· 未配置 Token：「未配置 GitHub Token」→「🧭 小纸船还没拿到罗盘，请先在「云同步」中设置」',
      '· 分享上传失败：「上传失败：xxx」→「🌊 小纸船在风暴里翻了：xxx」',
      '· 分享加载失败：「加载失败：xxx」→「🌫️ 小纸船没找到这封信：xxx」',
      '· 分享导入失败：「导入失败：xxx」→「🌫️ 小纸船没接住：xxx」',
      '· 云端恢复失败：「恢复失败：xxx」→「🌊 小纸船在风暴里迷路了：xxx」',
      '',
      '【💡 E1 — 提示词模板（输入栏 💡 按钮）】',
      '· 输入栏图片按钮右边新增「💡 提示词」按钮，点击从底部弹出 Sheet 面板（液态玻璃风格）',
      '· 内置 10 个 Coding 高频场景预设：',
      '  🌐 翻译成中文 / 🌏 翻译成英文 / 📖 解释这段代码 / 🔧 重构这段代码 / 🐛 帮我找 Bug',
      '  ✅ 写单元测试 / 📝 加注释 / 📋 写 commit message / 🎨 优化样式 / 📊 总结这段内容',
      '· 点击预设 → 自动填入输入框，光标定位到末尾（已有内容自动换行追加）',
      '· 「➕ 添加自定义」按钮 → 编辑器 modal（图标 + 名称 + 内容三项）',
      '· 长按/hover 显示「✕」删除按钮（移动端默认常显）',
      '· 「🔄 恢复默认」一键清空自定义、还原内置 10 个',
      '· 存 localStorage `codingplan-prompts`，ESC 关闭',
      '',
      '【▶️ E3 — 代码块实时预览（iframe sandbox）】',
      '· html / css / javascript / js 代码块右上角新增 ▶️ 预览按钮（其他语言不显示）',
      '· 点击在代码块下方展开 iframe sandbox 渲染区（默认 280px 高，可拖动调整）',
      '· HTML 直接渲染；CSS 自动套示例骨架（标题/段落/按钮/列表/.box）；JS 自动注入 console.log/warn/error 捕获器 + window.onerror 显示到日志区',
      '· iframe 安全策略：`sandbox="allow-scripts"` 不给 same-origin、不传 cookie/referrer → 预览代码无法访问主页 localStorage / Cookie',
      '· 工具栏：🔄 刷新 / ✕ 关闭预览（再次点 ▶️ 也能收起）',
      '· 橙色强调风格，与 Liquid 主题融合，展开/收起带 spring 弹性动画',
      '',
      '【🔧 改动文件】',
      '· assets/js/13-image.js · assets/js/15-tts.js · assets/js/16-providers.js',
      '· assets/js/17-fileupload.js · assets/js/19-configsync.js · assets/js/20-share.js',
      '· assets/js/11-input.js（代码块预览按钮注入）· assets/js/22-prompts.js（新）· assets/js/23-preview.js（新）',
      '· assets/css/main.css（typing-label + prompts-sheet + preview 三组样式）· index.html · 10-changelog.js',
    ]},
